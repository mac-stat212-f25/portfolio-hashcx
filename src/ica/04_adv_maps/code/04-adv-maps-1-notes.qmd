...
transform_and_plot(nc, new_crs = "EPSG:32031")
...
```{r}
ggplot() +
  geom_raster(data = elev_df, mapping = aes(x = x, y = y, fill = elevation)) +
  ...
  geom_sf(data = mn_cities |> filter(Population > 10000), mapping = aes(size = Population, color = Population), alpha = .5) +
  theme_map() +
