...
bind_rows(results1, results2, results3, results4, results5) |>
  mutate(type = c("for_loop_no_storage", "for_loop_with_storage", "purrr_map", "standard_func", "standard_slice")) |>
...
diamonds |> 
  ...
  map(sum_stats) |> 
  bind_rows() |> 
  mutate(variable = colnames(diamonds |> select(where(is.numeric))))
```
...
```{r}
str_data_2 <- tibble(
  string = c("Amin", "Macalester", "MSCS"),
  start = c(1, 2, 1),
  end = c(3, 4, 4)
)

pmap(str_data_2, str_sub)
pmap(str_data_2 |> select(-string), str_sub, string = "this is a test")
```
...
by_clarity <- by_clarity |> 
  mutate(path = str_glue("diamonds/diamonds-{clarity}.png"))
...
walk2( 
  by_clarity$path,
  by_clarity$plot,
  function(pt, pl){ ggsave(pt, pl, width = 6, height = 6)}
)
