...
ggplot(weather) +
  theme_classic() +
  geom_linerange(mapping = aes(x = dateInYear, ymin = RecordLow, ymax = RecordHigh), color = "#ECEBE3")
```
...
ggplot(weather) +
  theme_classic() +
  geom_linerange(mapping = aes(x = dateInYear, ymin = RecordLow, ymax = RecordHigh), color = "#ECEBE3", linewidth = 1) +
  geom_linerange(mapping = aes(x = dateInYear, ymin = NormalLow, ymax = NormalHigh), color = "#C8B8BA", linewidth = 1) +
  geom_linerange(mapping = aes(x = dateInYear, ymin = Low, ymax = High), color = "#A90248", linewidth = 1) 
```
...
ggplot(weather) +
  ...
  geom_linerange(mapping = aes(x = dateInYear, ymin = NormalLow, ymax = NormalHigh), color = "#C8B8BA", linewidth = 1) +
  geom_linerange(mapping = aes(x = dateInYear, ymin = Low, ymax = High), color = "#A90248", linewidth = 1) +
  geom_vline(xintercept = weather |> filter(Day == 1) |> pull(dateInYear), color = "gray", linetype = "dotted")
```
...
ggplot(weather) +
  ...
  geom_linerange(mapping = aes(x = dateInYear, ymin = Low, ymax = High), color = "#A90248", linewidth = 1) +
  geom_vline(xintercept = weather |> filter(Day == 1) |> pull(dateInYear), color = "gray", linetype = "dotted") +
  scale_x_continuous(breaks = weather |> filter(Day == 1) |> pull(dateInYear), labels = month.abb) +
...
ggplot(weather) +
  theme_classic() +
  geom_area(mapping = aes(x = dateInYear, y = CulmPrec), color = "#32a3d8", fill = "#ebeae2") + 
  geom_text(data = weather |> select(dateInYear, Month, CulmPrec) |>  group_by(Month) |> mutate(TotalPrec = max(CulmPrec)) |> select(-CulmPrec) |> group_by(Month) |> slice_tail(n = 1), mapping = aes(x = dateInYear, y = TotalPrec, label = TotalPrec), size = 3, vjust = -.5, hjust = 1) +
  geom_point(data = weather |> filter(RecordP), mapping = aes(x = dateInYear, y = CulmPrec), shape = "triangle down filled", fill = "black")
```
## Done!
- Render your notes.
