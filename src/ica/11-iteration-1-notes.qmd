...
```{r}
lin_model_res <- vector(mode = "list", length = nrow(df_arg_combos))

for(i in seq_along(lin_model_res)) {
  formula <- df_arg_combos$mod_formula[[i]]
  outliers <- df_arg_combos$remove_outliers[i]
  lin_model_res[[i]] <- fit_model(
    data = diamonds,
    mod_formula = formula,
