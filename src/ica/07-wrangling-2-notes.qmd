...
```{r load_regex_data}
data(fruit)
sample(fruit, size = 10) |> str_view(pattern = "a", match = TRUE)

data(words)
sample(words, size = 10) |> str_view(pattern = "a")

data(sentences)
sample(sentences, size = 10) |> str_view(pattern = "a", html = FALSE)
```
...
posts |> 
    ...
    ggplot() +
    geom_histogram(aes(x = title_len))
```
